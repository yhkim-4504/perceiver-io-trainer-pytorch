...
input_preprocessing_type = {'min_max_normalize': 'min_max_normalize', 'standardization': 'standardization', 'custom_std': 'custom_std'}  # dataset preprocessing type
...
dset_config = EasyDict(
    {
        ...
        'xyzs_path': r'C:\Users\home\Documents\Workspace\github\gap\data\dipeptides',  # xyzs path
        ...
        'use_dipeps': ['EN', 'MM'],  # when use_all_dipeps is False -> only this sequcenes will be used.
        'max_dset_ratio': 0.5,  # used dataset ratio just to use small dataset.
        'train_val_split': 0.5,  # train - valid split ratio
        'val_test_split': 0.7,  # valid - test split ratio
        'input_preprocessing': input_preprocessing_type['custom_std'],  # x(descriptor) preprocess
        'output_preprocessing': output_preprocessing_type['y0_subtract']  # y(total_energy) preprocess : subtract y0 or mean(y) of each sequence
    }
)
